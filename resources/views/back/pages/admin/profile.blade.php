...
@section('pageTitle',isset($pageTitle) ? $pageTilte : 'Profile')
@section('content')
    ...
    <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 mb-30">
        <div class="pd-20 card-box height-100-p">
            ...
            <div class="profile-photo">
                ...
                ('adminProfilePhotoFile').click();" class="edit-avatar"><i class="fa fa-pencil"></i></a>
                <img src="  {{ $admin->photo }} " alt="" class="avatar-photo" id="adminProfilePhoto">
                <input type="file" name="adminProfilePhotoFile" id="adminProfilePhotoFile" class="d-none" 
                style="opacity: 0">
            </div>
...
@push('scripts')
    <script>
        window.addEventListener('updateAdminInfo', function(event){
            ...
            // $('#AdminProfileUsername').html(event.detail.adminUsername);
        });
        $('input[type="file"][name="adminProfilePhotoFile"][id="adminProfilePhotoFile"]').ijaboCropTool({
          preview : '#adminProfilePhoto',
          setRatio:1,
          allowedExtensions: ['jpg', 'jpeg','png'],
          processUrl:'{{ route("admin.change-profile-photo") }}',
          withCSRF:['<?= csrf_token() ?>',],
          onSuccess:function(message, element, status){
            Livewire.emit('UpdateAdminSellerHeaderProfileInfo');
            alert(message);
          },
          onSuccess:function(message, element, status){
             alert(message);
